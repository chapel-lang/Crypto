...
/*  A cryptographic library based on 'OpenSSL'.
    ...
    The Crypto module focuses on providing various cryptographic utilities such
    as
...
module Crypto {
  ...
  class BaseCryptoBuffer {
    ...
    @chpldoc.nodoc
    proc getBuffDomain(): domain(1) {
      halt("getBuffDomain is not implemented in the base class");
      var dom: domain(1);
      return dom;
    }
    ...
    /* Returns the pointer to the entire internal buffer with
       ...
       :rtype: `c_ptr([] uint(8))`
    */
    proc getBuffPtr(): c_ptr(uint(8)) {
      halt("getBuffPtr is not implemented in the base class");
      return nil;
    }
    ...
    */
    proc getBuffSize(): int {
      halt("getBuffSize is not implemented in the base class");
      return -1;
    }

    @chpldoc.nodoc
    proc getFromBuffer(idx: int): uint(8) {
      return getBuffPtr()[idx];
    }
    ...
    */
    proc toHex() {
      var buffHex: [getBuffDomain()] string;
      for i in buffHex.domain {
        const byte = getFromBuffer(i);
  ...
  */
  class CryptoBuffer: BaseCryptoBuffer {
    ...
    */
    proc init(s: string) {
      ...
      if this._len == 0 then
        halt("Enter a string with length greater " +
    ...
    */
    proc init(s: [] uint(8)) {
      ...
      this._len = s.size;
      if this._len == 0 then
        halt("Enter an array with size greater " +
             "than 0 in order to create a buffer");
    ...
    */
    ...
    @chpldoc.nodoc
    override proc getBuffDomain() {
      return this.buffDomain;
    }
    ...
    */
    ...
    /* Returns the length of the entire internal buffer.
       ...
       :rtype: `int`
    */
    override proc getBuffSize(): int {
      return this._len;
    }

    @chpldoc.nodoc
    override proc getFromBuffer(idx: int): uint(8) {
  ...
  class LocalBorrowedCryptoBuffer: BaseCryptoBuffer {
    ...
    */
    proc init(ref s: string) {
      init this;
      this._len = s.numBytes;
      if this._len == 0 then
        halt("Enter a string with length greater " +
             "than 0 in order to create a buffer");
      this.buffPtr = c_ptrTo(s):c_ptr(uint(8));
    }
    ...
    */
    ...
    /* The `CryptoBuffer` class initializer that initializes the buffer
       ...
       :rtype: `CryptoBuffer`
    */
    proc init(ref s: [] uint(8)) {
      init this;
      this._len = s.size;
      if this._len == 0 then
        halt("Enter an array with size greater " +
             "than 0 in order to create a buffer");
      this.buffPtr = c_ptrTo(s);
    }

    /* Returns a domain that can be used to store the buffer as an array

      :return: the domain of the internal buffer representation.
      :rtype: `domain(1)`
    */
    override proc getBuffDomain() {
      return {0..<this._len};
  ...
  /* `RSAKey` class encloses the `EVP_PKEY` object provided by the
     ...
     functions may result in generation of a single object that contains both
     the keys (public and private).
  ...
  */
  class RSAKey {
    ...
    /* The `RSAKey` class initializer that initializes the `EVP_PKEY` object
       ...
       It checks for valid RSA key lengths and generates a public key and
       private key pair accordingly.
    ...
    */
    proc init(keyLen: int) {
      init this;
      if keyLen != 1024 && keyLen != 2048 && keyLen != 4096 then
  ...
  */
  class Envelope {
    ...
    */
    proc init(iv: owned CryptoBuffer,
              encSymmKey: [] owned CryptoBuffer,
              encSymmValue: owned CryptoBuffer) {
      init this;
    ...
    */
    ...
    /* This function returns a particular symmetric key buffer based
       ...
       .. note::
         ...
         The supplied index should be in the domain of the RSA-encrypted keys
         array.
  ...
  @chpldoc.nodoc
  record digestPrimitives {
    var ctx: CHPL_EVP_MD_CTX;
    var md: CONST_EVP_MD_PTR;
    var hashLen: int;
    proc type create(digestName: string, hashLen: int) {
      CHPL_OpenSSL_add_all_digests();

      var ctx = CHPL_EVP_MD_CTX_new();
      var md = EVP_get_digestbyname(digestName.c_str());
      var digestObj = new digestPrimitives(ctx, md, hashLen);

      EVP_DigestInit_ex(CHPL_EVP_MD_CTX_ptr(digestObj.ctx), md, nil:ENGINE_PTR);

      return digestObj;
    }
    proc ref update(inputBuffer: borrowed BaseCryptoBuffer) {
      EVP_DigestUpdate(CHPL_EVP_MD_CTX_ptr(ctx),
                       inputBuffer.getBuffPtr(): c_ptr(void),
                       inputBuffer.getBuffSize(): c_size_t);
    }
    proc ref finish() {
      var retHashLen: c_uint = 0;
      var hash: [0..#hashLen] uint(8);
      EVP_DigestFinal_ex(CHPL_EVP_MD_CTX_ptr(ctx),
                         c_ptrTo(hash): c_ptr(c_uchar),
                         retHashLen);
  ...
  */
  class Hash {
    ...
    */
    proc init(digestName: Digest) {
      ...
      select digestName {
        ...
        otherwise do halt("A digest with the name \'" +
                          digestName: string +
                          "\' doesn't exist.");
      }
    ...
    */
    proc getDigest(inputBuffer: borrowed BaseCryptoBuffer): owned CryptoBuffer {
      var obj = digestPrimitives.create(this.digestName, this.hashLen);
      obj.update(inputBuffer);
      this.hashSpace = obj.finish();
      var hashBuffer = new CryptoBuffer(this.hashSpace);
      return hashBuffer;
    }
  }

  /* The `StreamingHash` class provides the same hash functions as the `Hash`
     class but with a different interface. It is useful in cases where the
     input data is too large to be stored in memory and needs to be processed
     in chunks. It provides `update` and `finish` functions that can
     be used to hash the input data in a streaming manner.
  */
  class StreamingHash: Hash {
    @chpldoc.nodoc
    ...
    override proc getDigest(
      inputBuffer: borrowed BaseCryptoBuffer
    ): owned CryptoBuffer {
      halt("StreamingHash does not support the getDigest function. " +
           "Use the update and finish functions instead.");
      var dummy = new CryptoBuffer("");
      return dummy;
    }
  }

  @chpldoc.nodoc
  proc aesEncrypt(plaintext: CryptoBuffer,
                  key: CryptoBuffer,
                  IV: CryptoBuffer,
    ...
    var ciphertextLen: c_int =
      plaintextLen + 16; // 16 is the MAX_BLOCK_SIZE for AES
  ...
  @chpldoc.nodoc
  proc aesDecrypt(ciphertext: CryptoBuffer,
                  key: CryptoBuffer,
                  IV: CryptoBuffer,
                  cipher: CONST_EVP_CIPHER_PTR) {
  ...
  /* The `CryptoChainMode` enum represents all cipher chaining modes
     that can be used by a symmetric cipher. It is used by the `AES` class
     initializer to select a chaining mode.
  */
  enum CryptoChainMode {
    cbc, ecb, cfb, ofb
  }

  /* The `AES` class represents the symmetric encryption algorithm, AES. The
     ...
     Currently, the `AES` class allows symmetric encryption using only the CBC
     or Cipher Block Chaining mode in 128, 192, and 256 key size variants.

     After thorough testing, ECB, OCB, and other chaining mode variants will
     also be added to this library(TODO).
  ...
  */
  class AES {
    ...
    */
    ...
    /* This is the 'AES' encrypt routine that encrypts the user supplied message
       buffer using the key and IV.

       The `encrypt` takes in the plaintext buffer, key buffer, and IV buffer as
       the arguments and returns a buffer of the ciphertext.

       :arg plaintext: A `CryptoBuffer` representing the plaintext to be
                       encrypted.
       :type plaintext: `CryptoBuffer`

       :arg key: A `CryptoBuffer` representing the key to be used for
                 encryption.
       :type key: `CryptoBuffer`

       :arg IV: A `CryptoBuffer` representing the initialization vector to be
                used for encryption.
    ...
    */
    proc encrypt(plaintext: CryptoBuffer,
                 key: CryptoBuffer,
                 IV: CryptoBuffer): owned CryptoBuffer {
    ...
    /* This is the 'AES' decrypt routine that decrypts the user supplied
       ciphertext buffer using the same key and IV used for encryption.

       The `decrypt` takes in the ciphertext buffer, key buffer, and IV buffer
       as the arguments and returns a buffer of the decrypted plaintext.

       :arg ciphertext: A `CryptoBuffer` representing the ciphertext to be
                        decrypted.
       ...
       :arg IV: A `CryptoBuffer` representing the initialization vector to be
                used for decryption (same as the one used in encryption).
       :type IV: `CryptoBuffer`
  ...
  @chpldoc.nodoc
  proc bfEncrypt(plaintext: CryptoBuffer,
               key: CryptoBuffer,
               IV: CryptoBuffer,
               cipher: CONST_EVP_CIPHER_PTR) {
  ...
  @chpldoc.nodoc
  proc bfDecrypt(ciphertext: CryptoBuffer,
                 key: CryptoBuffer,
                 IV: CryptoBuffer,
                 cipher: CONST_EVP_CIPHER_PTR) {
  ...
  */
  class CryptoRandom {
    /* This function represents a CSPRNG that generates and allocates the
    ...
    proc getRandomBuffer(buffLen: int): owned CryptoBuffer throws {
      if buffLen < 1 then
        throw new IllegalArgumentError(
         "illegal argument 'buffLen': Invalid random buffer length specified.");
  ...
  */
  class KDF {
    ...
    */
    proc passKDF(userKey: string, saltBuff: CryptoBuffer): owned CryptoBuffer {
      var key = PBKDF2(userKey, saltBuff, this.byteLen,
                       this.iterCount, this.hashName);
  ...
  @chpldoc.nodoc
  proc rsaEncrypt(keys: [] RSAKey, plaintext: CryptoBuffer,
                  ref iv: [] uint(8), ref encSymmKeys: [] CryptoBuffer) {
  ...
  @chpldoc.nodoc
  proc rsaDecrypt(key: RSAKey, iv: [] uint(8), ciphertext: [] uint(8),
                  encKeys: [] CryptoBuffer) throws {
    ...
    if !openErrCode then
      throw new IllegalArgumentError(
        "illegal argument 'key': The RSAKey is an invalid match.");
  ...
  /* RSA is one of the first practical public-key cryptosystems and is widely
      used for secure data transmission. A user of the `RSA` first generates
      the public and private keys using the `RSAKey` class. The `RSAKey`
      object is then passed to the `RSA` functions for further encryption and
      decryption purposes.

      The `encrypt` function of this class takes in an array of `RSAKey`
      objects and uses all of them to perform the encryption.

      Similarly, decryption can be done individually performed (without
      passing an array as an argument) by passing a single `RSAKey` object
      every time the decryption takes place.

  */
  class RSA {

    /* This is the 'RSA' encrypt routine that encrypts the plaintext buffer.
       This uses the `AES` encryption algorithm to encrypt the plaintext and
       auto-generates a secure IV and key. Furthermore, the key is encrypted
       using `RSA` based on the 'RSAKey' objects supplied in the arguments.

       The function returns an `Envelope` object that encloses the
       auto-generated IV, AES encrypted ciphertext, and an array of `RSA`
       encrypted key buffers. The number of encrypted keys is equal to the
       number of `RSAKey` objects in the array. This kind of use case is useful
       specifically in developing one-to-many systems such as GPG.

       For instance, consider a server-client model where the server performs
       encryption using the public keys of all the clients and returns an
       `Envelope` to each client. Now, the decryption can only be performed by
       each client using only their own private key.
    ...
    */
    proc encrypt(plaintext: CryptoBuffer, keys: [] RSAKey): owned Envelope {
      ...
      var envp = new Envelope(
          new CryptoBuffer(iv), encSymmKeys, new CryptoBuffer(ciphertext));
      return envp;
    }

    /* This is the 'RSA' decrypt routine that decrypts the ciphertext buffer.
       This uses `AES` decryption to decrypt the ciphertext.
  ...
  */
  module C_OpenSSL {
    ...
    require "openssl_c_support.h";

    require "openssl/pem.h", "openssl/bn.h", "openssl/bio.h", "openssl/evp.h",
            "openssl/aes.h", "openssl/rand.h", "openssl/sha.h";
